    ...
    **kwargs,  # Additional tokenizer kwargs passed to constructor
    ...
    if schema == "local-dir":
        ...
        if not local_dir_path.is_dir():
            raise FileNotFoundError(
                f"Directory specified via 'local-dir:' schema not found: {local_dir_path}"
            )

        local_config_path = local_dir_path / "open_clip_config.json"
        logging.info(
            f"Attempting to load config from {config_source_description} at {local_config_path}"
        )
        if local_config_path.is_file():
            try:
                ...
                raise ValueError(
                    f"Could not load valid config from 'local-dir:{identifier}': {e}"
                ) from e
        else:
            raise FileNotFoundError(
                f"'local-dir:' specified, but config file missing: {local_config_path}"
...
def create_model_and_transforms(
    ...
    **model_kwargs,
    ...
    logger.info(f"Creating model '{model_name}' with pretrained='{pretrained}'...")
    ...
    logger.info("Preprocessing transforms created successfully.")
